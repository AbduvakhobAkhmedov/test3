...
class Utilities
{
	public function getPaging($page, $total_rows, $record_per_page, $page_url)
	{
		...
		// подсчет всех товаров в БД для подчета общего количество страниц
		$total_pages = ceil($total_rows / $records_per_page);

		// диапазон ссылок для показа
		$range = 2;

		// отображфть диапазон ссылок вокруг текущей страницы
		$initial_num = $page - $range;
		$condition_limit_num = ($page + $range) +1;
		$paging_arr["pages"] = array();
		$page_count = 0;

		fox ($x = $initial_num; $x < $condition_limit_num; $x++) {
